defmodule Standalone do
  ...
  def copy_elixir(release, elixir_version) do
    ...
    elixir_path = Path.join(File.cwd!(), "rel/_build/elixir-rel-#{elixir_version}")

    cp_r!(elixir_path, standalone_destination)
  ...
  defp download_url!(url, path) do
    Logger.info("Downloading #{url} to #{path}")

    {output, status} = System.cmd("curl", [
      "-vL", "-X", "GET",
      url, "-o", path
    ])
    Logger.debug(output)
    ...
    unless status == 0 do
      Logger.error("couldn't fetch #{url} (#{inspect status})")
      raise "couldn't fetch #{url} (#{inspect status})"
    end
  end

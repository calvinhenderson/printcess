defmodule PrintClientWeb.PrinterSelect do
        ...
        <div class="dropdown w-full">
          ...
          <ul tabindex="0" class="dropdown-content menu p-2 shadow-lg bg-base-100 border-neutral-content border-2 rounded-box w-full">
            <%= for printer <- @printers do %>
              <li><button tabindex="0" phx-click="select-printer" phx-value-printer={printer.id} class={ if @current_printer.id == printer.id, do: "btn-disabled", else: ""}><%= printer.name %></button></li>
            <% end %>
          </ul>
        </div>

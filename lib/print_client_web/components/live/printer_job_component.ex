defmodule PrintClientWeb.PrinterJobComponent do
  ...
  def render(assigns) do
    ~H"""
    <div id={@id} class="collapse collapse-arrow bg-base-100 border-base-300 border">
      <input type="checkbox" />
      <div class="collapse-title font-semibold flex justify-between items-center">
        <span class="inline-block">{@job.printer.name}</span>
        <progress
          class={[
            "progress w-56",
      ...
      <div class="collapse-content space-y-4 text-right">
        <.button phx-target={@myself} phx-click="resend" phx-disable-with="Resending.." disabled={not @complete}>
          <.icon name="hero-arrow-path" /> Resend
        </.button>
        <.button phx-target={@myself} phx-click="cancel" phx-disable-with="Cancelling.." disabled={@complete}>
  ...
  @impl true
  def handle_event("cancel", _params, %{assigns: %{job: %{id: job_id, printer: printer}}} = socket) do
  ...
  @impl true
  def handle_event("resend", _params, %{assigns: %{job: %{printer: printer, template: template, params: params}}} = socket) do
    Printer.add_job(printer.printer_id, template, params)
    {:noreply, socket}
  end

  defp assign_job(%{assigns: %{job: job}} = socket) do
    topic = Printer.topic(job.printer, job.id)
  ...
  defp assign_job_status(%{assigns: %{job: job}} = socket, :fetch) do
    status =
      case Printer.get_job(job.printer.printer_id, job.id) do
